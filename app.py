...
# Inject current view as body class to scope CSS
st.markdown(f'<script>document.body.setAttribute("data-view", "{st.session_state.view}")</script>', unsafe_allow_html=True)

# Try to restore session from Supabase on page refresh
...
def render_sidebar():
    """Persistent sidebar — profile info and sign out."""
    ...
    with st.sidebar:
        ...
        col1.metric("Total", total_trips)
        col2.metric("Upcoming", upcoming)
        col3, col4 = st.columns(2)
        col3.metric("Ongoing", ongoing)
        col4.metric("Completed", completed)
        st.divider()
        if st.button("Sign Out", type="secondary", use_container_width=True, key="sidebar_signout"):
            auth.logout()
...
def show_dashboard():
    ...
    with prof_col:
        st.markdown('<div class="dashboard-profile-btn">', unsafe_allow_html=True)
...
def trip_creator_dialog():
    ...
    with budget_col:
        max_budget = st.number_input("Optional | Max budget (in USD) 0 = no limit", min_value=0, value=0, step=50)
    ...
    if st.button("Generate My Plan", use_container_width=True, type="primary"):
        ...
        if max_budget > 0:
            # Figure out how much hotel costs per night
            if chosen_hotel:
                hotel_nightly = df[df['name'] == chosen_hotel]['cost'].values[0]
            else:
                hotel_pool = df[(df['city'] == target_city) & (df['category'] == 'Hotel')]
                hotel_nightly = hotel_pool['cost'].min() if not hotel_pool.empty else 0

            hotel_total = hotel_nightly * trip_days
            misc_total  = 40 * trip_days
            activity_budget = max_budget - hotel_total - misc_total

            if activity_budget < 0:
                # Hotel alone exceeds budget — suggest cheapest hotel
                cheapest = df[(df['city'] == target_city) & (df['category'] == 'Hotel')].nsmallest(1, 'cost')
                if not cheapest.empty:
                    cheapest_name = cheapest.iloc[0]['name']
                    cheapest_cost = int(cheapest.iloc[0]['cost'])
                    st.warning(f"Your budget of ${max_budget} doesn't cover the hotel alone. Consider **{cheapest_name}** at ${cheapest_cost}/night instead.")
                    st.stop()

            # Filter non-hotel spots to only those affordable within remaining budget
            # Sort by cost ascending so cheapest spots are prioritised
            non_hotel_mask = filtered['category'] != 'Hotel'
            filtered_activities = filtered[non_hotel_mask].sort_values('cost')
            # Keep spots whose cumulative cost fits within activity_budget
            filtered_activities = filtered_activities[filtered_activities['cost'] <= max(activity_budget / max(trip_days, 1), 0)]
        ...
        # Budget feedback after generation
        if max_budget > 0 and cost > max_budget:
            over_by = cost - max_budget
            st.warning(f"Heads up — the estimated cost is ${cost:,.0f}, which is ${over_by:,.0f} over your ${max_budget:,.0f} budget. Consider fewer days or a cheaper hotel.")
        ...
        new_trip = {
            ...
            "status": auto_status, "max_budget": max_budget if max_budget > 0 else None
        }
        try:
...
def show_details():
    ...
    with tab2:
        ...
        if len(coords) > 1:
            try:
                r = requests.get(
                    f"http://router.project-osrm.org/route/v1/driving/{';'.join(coords)}?overview=full&geometries=geojson&steps=true&annotations=true",
                    timeout=5).json()
                if r.get('routes'):
                    ...
                    # Get snapped waypoint coordinates from OSRM
                    if r.get('waypoints'):
                        snapped_coords = [(wp['location'][1], wp['location'][0]) for wp in r['waypoints']]
            except:
                pass
        ...
        if route_geometry:
            # Main route line
            folium.PolyLine(
                [[p[1], p[0]] for p in route_geometry],
        ...
        for i, (_, row) in enumerate(route_data.iterrows()):
            if snapped_coords and i < len(snapped_coords):
                ...
                if abs(snapped[0] - actual[0]) > 0.0001 or abs(snapped[1] - actual[1]) > 0.0001:
                    folium.PolyLine(
                        [snapped, actual],
